class Solution:
    def maximumSubarraySum(self, nums: list[int], k: int) -> int:
        ...
        for i in range(first_item_index , last_item_index + 1):
            current_sum += nums[i]
            item_exists_in_hash_map = hash_map[nums[i]] 
        ...
        while last_item_index < len(nums)-1:
            ...
            incremented_item_exists_or_not = hash_map[nums[last_item_index+1]]
            if incremented_item_exists_or_not > 0:
                duplicate_item_number += 1
            hash_map[nums[last_item_index+1]] = hash_map[nums[last_item_index +1]] + 1
            decremented_item_exists_or_not = hash_map[nums[first_item_index]]
